        ...
        <div class="w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl ring-1 ring-slate-300/50 flex flex-col md:flex-row bg-white">
            ...
            <div class="flex-1 p-6 sm:p-8 md:p-10 flex flex-col justify-center">
                ...
                @if(config('services.google.client_id'))
                    ...
                    <a href="{{ route('auth.google.redirect', ['for' => $for ?? 'resident', 'tenant_id' => $tenant->id, 'intent' => ($for ?? '') === 'resident' ? 'signup' : 'login']) }}" class="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-700 shadow-sm transition hover:bg-slate-50">
