...
<head>
    ...
    <title>@yield('title', 'Resident') – {{ $brandName ?? config('bhcas.name') }}</title>
    <script>
...
<body class="min-h-screen bg-slate-50 text-slate-800 antialiased {{ $themeClass }} @if($navLayout === 'sidebar')layout-sidebar @endif" data-theme="{{ e($tenant?->theme ?? 'default') }}" data-nav-layout="{{ e($navLayout) }}" @if($tenant) data-tenant-id="{{ $tenant->id }}" @endif>
    ...
    <aside class="tenant-brand-nav sidebar-drawer fixed left-0 top-0 z-40 h-full w-56 border-r border-white/10 bg-teal-600 shadow-lg md:translate-x-0" id="resident-nav" data-brand-color="{{ e($brandColor) }}">
        <div class="flex h-14 items-center justify-between gap-2 border-b border-white/20 px-4">
            <div class="flex min-w-0 items-center gap-2">
                ...
                <span class="truncate font-semibold text-white">{{ $brandName }} – Resident</span>
            </div>
        ...
        <nav class="flex flex-col gap-1 overflow-y-auto p-3">
            ...
            <a href="{{ route($item['route']) }}" class="{{ request()->routeIs($item['route'] . '*') || request()->routeIs($item['route']) ? $rsbActive : $rsbDefault }}">{{ $item['label'] }}</a>
            @endforeach
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex h-14 min-w-0 items-center justify-between gap-2">
                <a href="{{ route('resident.dashboard') }}" class="flex min-w-0 shrink-0 items-center gap-2 font-semibold text-white">
                    ...
                    <span class="truncate">{{ $brandName }} – Resident</span>
                </a>
                <div class="relative shrink-0">
                    ...
                    <div id="resident-menu-dropdown" class="nav-dropdown-panel absolute right-0 top-full z-10 mt-1 hidden min-w-[180px] rounded-lg bg-white py-1 shadow-lg ring-1 ring-black/5">
                        @foreach($residentNavItems as $item)
                        <a href="{{ route($item['route']) }}" class="block px-4 py-2 text-sm {{ request()->routeIs($item['route'] . '*') || request()->routeIs($item['route']) ? 'bg-teal-50 font-medium text-teal-700' : 'text-slate-700 hover:bg-slate-50' }}">{{ $item['label'] }}</a>
                        @endforeach
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex h-14 min-w-0 items-center justify-between gap-2">
                ...
                <div id="resident-nav-links" class="hidden flex-wrap items-center gap-1 md:flex">
                    @foreach($residentNavItems as $item)
                    <a href="{{ route($item['route']) }}" class="{{ request()->routeIs($item['route'] . '*') || request()->routeIs($item['route']) ? $rNavActive : $rNavDefault }}">{{ $item['label'] }}</a>
                    @endforeach
            ...
            <div id="resident-nav-mobile" class="hidden border-t border-white/20 px-4 py-3 md:hidden">
                <div class="flex flex-col gap-1">
                    @foreach($residentNavItems as $item)
                    <a href="{{ route($item['route']) }}" class="{{ request()->routeIs($item['route'] . '*') || request()->routeIs($item['route']) ? $rMobActive : $rMobDefault }}">{{ $item['label'] }}</a>
                    @endforeach
    ...
    <main class="px-4 py-8 sm:px-6 lg:px-8 @if($navLayout === 'sidebar') main-with-sidebar pt-14 md:pt-8 @else mx-auto max-w-4xl @endif">
        @if(session('success'))
