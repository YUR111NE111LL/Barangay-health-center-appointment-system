...
@section('title', 'Audit log — '.$tenant->name)

@section('content')
<div class="mb-6">
    <a href="{{ route('super-admin.tenants.show', $tenant) }}" class="text-sm font-medium text-violet-600 hover:text-violet-700">← Back to tenant</a>
</div>
...
@if(!empty($auditLogTableMissing))
    <div class="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        ...
        <p class="mt-1 text-amber-800">Run tenant migrations (for example: <code class="rounded bg-amber-100 px-1 py-0.5 text-xs">php artisan tenants:migrate</code>) or use <strong>Provision Tenant DB</strong> from this tenant’s page if the database was created before audit logging was added.</p>
    </div>
    <div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50/90 px-6 py-14 text-center shadow-sm">
        ...
        <p class="text-sm font-medium text-slate-800">No audit table in this tenant database yet</p>
        <p class="mx-auto mt-2 max-w-lg text-sm leading-relaxed text-slate-600">Once the <code class="rounded bg-white px-1 py-0.5 text-xs ring-1 ring-slate-200">audit_logs</code> table exists, entries will appear here.</p>
    </div>
@else
<div class="overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
    <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-4 py-10 text-center text-slate-500">No audit entries yet.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
