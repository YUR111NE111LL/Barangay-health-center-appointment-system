        ...
        @else
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <label class="flex cursor-pointer items-start gap-2 rounded-lg border border-slate-200 px-3 py-2 hover:bg-slate-50">
                    <input type="checkbox" name="permissions[]" value="{{ $p->name }}" {{ in_array($p->name, old('permissions', []), true) ? 'checked' : '' }} class="mt-1 h-4 w-4 shrink-0 rounded border-slate-300 text-teal-600 focus:ring-teal-500">
