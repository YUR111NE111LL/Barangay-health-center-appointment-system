...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800">Edit medicine</h1>
    <p class="mt-1 text-slate-500">{{ $medicine->name }}</p>
</div>

<form action="{{ route('backend.medicines.update', $medicine) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200/60">
        @include('tenant.medicines._form', ['medicine' => $medicine])
    </div>
