...
@section('content')
<div class="mb-6">
    <a href="{{ route('backend.profile.show') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 transition hover:text-slate-700">
...
<form action="{{ route('backend.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200/60">
        ...
        <div class="space-y-4">
            ...
            <div id="large-preview-container" class="hidden rounded-xl border-2 border-dashed border-teal-200 bg-teal-50/50 p-4">
                <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-teal-700">Preview</p>
                <div class="flex justify-center">
                    <img id="large-preview" src="" alt="Preview" class="max-h-48 max-w-full rounded-lg border-2 border-teal-300 object-cover shadow-md ring-2 ring-teal-400">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <a href="{{ route('backend.profile.show') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50">
            Cancel
        </a>
        <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-teal-700">
    ...
    if (fileInput && preview) {
        ...
        if (removeCheckbox && preview) {
            removeCheckbox.addEventListener('change', function() {
                if (this.checked) {
                    if (preview.tagName === 'IMG') preview.style.opacity = '0.5';
                    if (largePreviewContainer) largePreviewContainer.classList.add('hidden');
                } else {
                    if (preview.tagName === 'IMG') preview.style.opacity = '1';
                }
            });
        }
    }
