...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800">New Health Event</h1>
    <p class="mt-1 text-slate-500">Add vaccination drives, health seminars, or check-up schedules.</p>
</div>
<form action="{{ route('backend.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200/60">
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required class="w-full rounded-xl border-slate-300 bg-slate-50 px-4 py-2.5 focus:border-teal-500 focus:ring-teal-500">{{ old('description') }}</textarea>
                @error('description')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
            </div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label for="event_date" class="mb-1 block text-sm font-medium text-slate-700">Date *</label>
                    <input type="date" name="event_date" id="event_date" value="{{ old('event_date') }}" required class="w-full rounded-xl border-slate-300 bg-slate-50 px-4 py-2.5 focus:border-teal-500 focus:ring-teal-500">
                ...
                <div>
                    ...
                    <input type="time" name="event_time" id="event_time" value="{{ old('event_time') }}" class="w-full rounded-xl border-slate-300 bg-slate-50 px-4 py-2.5 focus:border-teal-500 focus:ring-teal-500">
            ...
            <div>
                ...
                <input type="text" name="location" id="location" value="{{ old('location') }}" class="w-full rounded-xl border-slate-300 bg-slate-50 px-4 py-2.5 focus:border-teal-500 focus:ring-teal-500">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Standard/Premium plan. Max 2MB.</p>
            ...
            <div class="flex flex-col gap-1">
                ...
                <p class="ml-6 text-xs text-slate-500">Sends one email per approved user with an email address in this barangay (same mail settings as appointment notifications). Only runs when the event is published.</p>
            </div>
        </div>
    </div>
    <div class="flex gap-3">
        <button type="submit" class="rounded-xl bg-teal-600 px-4 py-2.5 font-medium text-white hover:bg-teal-700">Create event</button>
